...
@unittest.skipUnless(
    observer.supported_platform(), "observer requires POSIX locks/signals"
)
class ObserverTest(unittest.TestCase):
    ...
    def plan(self, binding=None):
        return observer.plan(
            self.settings, self.state, self.repo, Path(sys.executable), binding
        )
    ...
    def test_privacy_unknown_dedup_and_lifecycle(self):
        ...
        self.assertEqual(
            h["sessions"][0]["lifecycle"], "incomplete_missing_start_and_end"
        )
        self.fire("SessionStart", source="resume")
        self.fire("Stop", prompt_id="prompt-2")
        self.assertEqual(
            observer.health(self.state)["sessions"][0]["lifecycle"],
            "open_or_incomplete",
        )
        self.fire("SessionEnd", reason="other")
        self.assertEqual(
            observer.health(self.state)["sessions"][0]["lifecycle"], "end_observed"
        )
        self.fire("SessionStart", source="resume")
        self.assertEqual(
            observer.health(self.state)["sessions"][0]["lifecycle"],
    ...
    def test_exact_binding_no_child_or_other_session_inheritance(self):
        self.install(
            {"session_id": "session-1", "agent_id": None, "actor_ref": "agent:known"}
        )
        self.assertEqual(self.fire()[0]["actor_ref"], "agent:known")
        self.assertIsNone(
            self.fire("PreToolUse", agent_id="child", tool_use_id="tool-1")[-1][
                "actor_ref"
            ]
        )
        self.assertIsNone(self.fire("Stop", session_id="other")[-1]["actor_ref"])
        self.assertEqual(
            observer.observations(self.state)[0]["attribution_basis"],
            "explicit_local_binding",
        )
    ...
    def test_writable_interpreter_and_ancestors_remain_untrusted(self):
        import shutil

        directory = self.root / "interpreter"
        directory.mkdir(mode=0o700)
        executable = directory / "python"
        shutil.copyfile(sys.executable, executable)
        executable.chmod(0o700)
        self.assertEqual(str(executable), observer.trusted_executable(executable))
        for target in (executable, directory):
            for mode in (0o720, 0o702):
                target.chmod(mode)
                with self.assertRaisesRegex(ValueError, "untrusted_executable_path"):
                    observer.trusted_executable(executable)
            target.chmod(0o700)
        # Script trust is separate from interpreter trust.
        script = directory / "hook.py"
        script.write_text("pass")
        script.chmod(0o620)
        with self.assertRaisesRegex(ValueError, "untrusted_executable_path"):
            observer.trusted_executable(script, executable=False)
    ...
    def test_settings_secret_not_copied_to_plan_or_manifest(self):
        ...
        observer.remove(self.state, consent=True)
        self.assertEqual(
            json.loads(self.settings.read_text())["env"]["TOKEN"], "PRIVATE_SENTINEL"
        )
    ...
    def test_missing_end_and_subagent_stop_do_not_complete_parent(self):
        ...
        self.assertEqual(
            observer.observations(self.state)[-1]["deduplication"], "unavailable"
        )
    ...
    def test_target_disabled_and_missing_owned_hooks_visible(self):
        ...
        self.install()
        self.assertEqual(
            observer.health(self.state)["registration"], "disabled_by_target_settings"
        )
        config = json.loads(self.settings.read_text())
    ...
    def test_spool_capacity_and_database_lock_are_neutral(self):
        ...
        self.install()
        data = json.dumps(
            {"hook_event_name": "Stop", "session_id": "one", "cwd": str(self.repo)}
        ).encode()
        m = observer.manifest(self.state)
        assert m is not None
    ...
    def test_crash_recovery_after_settings_apply_and_remove(self):
        ...
        with (
            patch.object(observer, "save_manifest", side_effect=crash_install),
        ...
        with (
            patch.object(observer, "save_manifest", side_effect=crash_remove),
            self.assertRaises(OSError),
        ):
            observer.remove(self.state, consent=True)
        self.assertEqual(self.fire(), [])
    ...
    def test_cas_recheck_rejects_edit_while_preparing(self):
        ...
        with (
            patch.object(observer, "save_manifest", side_effect=edit_settings),
            self.assertRaises(ValueError),
        ):
            observer.apply(p, consent=True)
        self.assertEqual(
            json.loads(self.settings.read_text()), {"customer": "concurrent"}
        )
        observer.apply(self.plan(), consent=True)
        self.assertEqual(
            json.loads(self.settings.read_text())["customer"], "concurrent"
    ...
    def test_cli_plan_apply_health_and_exact_actor_validation(self):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            ...
            self.assertEqual(
                main(["observer", "apply", "--plan", str(p), "--consent"]), 0
            )
            self.assertEqual(
                main(["observer", "health", "--state-dir", str(self.state)]), 0
            )
    ...
    def test_cli_binding_requires_exact_registered_actor_not_alias(self):
            ...
            self.assertEqual(
                main(
                    ...
                    + [
                        ...
                        "add",
                        "--ref",
                        "urn:fixture:actor:one",
                        "--display-name",
                        "One",
                        "--attester-ref",
                        "urn:fixture:attester:local",
                        "--alias",
                        "alias-one",
                    ]
                ),
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(
                main(prefix + ["observer", "apply", "--plan", str(p), "--consent"]), 0
            )
    ...
    def test_generated_shell_command_runs_isolated_handler(self):
        self.install()
        command = json.loads(self.settings.read_text())["hooks"]["SessionStart"][0][
            "hooks"
        ][0]["command"]
        result = subprocess.run(
            command,
