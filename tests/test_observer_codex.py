...
class CodexObserverTest(test_observer.ObserverTest):
    def setUp(self):
        ...
        self.probe = patch.object(
            observer, "discover", return_value=[{"support": "fixture_not_qualified"}]
        )
        self.probe.start()
    ...
    def test_pinned_codex_wire_fields_are_allowlisted(self):
        ...
        self.assertTrue(
            all(row["runtime"] == "codex" and row["actor_ref"] is None for row in rows)
        )
        before = len(rows)
...
class DiscoveryTest(unittest.TestCase):
    ...
    def test_unsupported_version_and_detected_interface(self):
        ...
        current = classify(
            "codex", "codex-cli 0.153.4", "--dangerously-bypass-hook-trust"
        )
        self.assertEqual(current["version"], "0.153.4")
        self.assertEqual(
            current["support"], "documented_interface_detected_runtime_unqualified"
        )
    ...
    def test_script_wrapper_never_executed(self):
        ...
        wrapper.chmod(0o700)
        with patch(
            "devdiary.observer_discovery.shutil.which", return_value=str(wrapper)
        ):
    ...
    def test_probe_credential_free_isolation_and_bounded_output(self):
        ...
        with (
            patch(
                "devdiary.observer_discovery.subprocess.Popen", wraps=subprocess.Popen
            ) as spawn,
            patch.dict(
                os.environ, {"OPENAI_API_KEY": "SECRET", "ANTHROPIC_API_KEY": "SECRET"}
            ),
        ):
        ...
        with (
            patch("devdiary.observer_discovery.LIMIT", 1),
    ...
    def test_windows_structured_unsupported(self):
        ...
        output = io.StringIO()
        with (
            patch.object(observer, "supported_platform", return_value=False),
            contextlib.redirect_stdout(output),
        ):
            self.assertEqual(
                main(["observer", "health", "--state-dir", str(self.state)]), 2
            )
        self.assertEqual(
            json.loads(output.getvalue())["error_code"], "observer_platform_unsupported"
        )
