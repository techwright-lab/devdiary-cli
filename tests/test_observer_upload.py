...
class UploadTest(unittest.TestCase):
    def install(self):
        observer.apply(
            observer.plan(self.settings, self.state, self.repo, sys.executable),
            consent=True,
        )
    ...
    def fire(self, event="SessionStart", **extra):
        ...
        data = dict(
            session_id="session-1", cwd=str(self.repo), hook_event_name=event, **extra
        )
    ...
    def setUp(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_POST(self):
                ...
                poisoned = (
                    owner.poison is not None and data.get("prompt_id") == owner.poison
                )
    ...
    def test_wrong_receipt_redirect_conflict_stay_pending(self):
        ...
        self.configure()
        self.fire()
        for status, bad, failure in [
            (201, True, "invalid_receipt"),
            (302, False, "http_302"),
            (409, False, "http_409"),
        ]:
    ...
    def test_crash_after_server_acceptance_restart_and_config_conflict(self):
        ...
        self.fire()
        with (
            patch.object(upload, "acknowledge", side_effect=RuntimeError("crash")),
        ...
        with self.assertRaises(ValueError):
            upload.configure(
                self.state,
                endpoint=self.url,
                collector_ref="collector:changed",
                repository_ref="https://github.com/owner/repo",
                key_file=self.key,
                consent=True,
            )
    ...
    def test_sql_cursor_bounded_batch_and_reset_preserves_receipts(self):
        ...
        with sqlite3.connect(self.state / "observations.sqlite3") as db:
            db.execute(
                "UPDATE observations SET metadata=json_set(metadata, '$.private', ?)",
                ("SECRET" * 10000,),
            )
        ...
        with sqlite3.connect(self.state / "observations.sqlite3") as db:
            self.assertEqual(
                db.execute("SELECT seq FROM upload_cursor").fetchone()[0], 5
            )
    ...
    def test_cli_configuration_sync_and_safe_failure(self):
        ...
        with contextlib.redirect_stdout(out):
            ...
            self.status = 401
            self.assertEqual(
                main(["observer", "sync", "--state-dir", str(self.state)]), 1
            )
            self.key.unlink()
            self.assertEqual(
                main(["observer", "sync", "--state-dir", str(self.state)]), 2
            )
    ...
    def test_deterministic_microseconds_and_payload_ceiling(self):
        config = {
            "repository": "/local",
            "installation_id": "i",
            "repository_ref": "https://github.com/a/b",
        }
        row = {
            "repository": "/local",
            "installation_id": "i",
            "observed_at": 1700000000.123456,
        }
        self.assertEqual(
            json.loads(upload.envelope(dict(row), config))["observed_at"],
    ...
    def assert_poison_fairness(self):
        ...
        self.assertEqual(
            [json.loads(raw)["prompt_id"] for raw in self.requests],
            ["p0", "p1", "p2", "p0"],
        )
        self.assertEqual(self.requests[0], self.requests[3])
        with sqlite3.connect(self.state / "observations.sqlite3") as db:
            rows = db.execute(
                "SELECT payload,delivered,failure FROM upload_outbox ORDER BY seq"
            ).fetchall()
    ...
    def test_global_failure_stops_batch_but_advances_on_restart(self):
        ...
        with patch.object(
            upload, "post", return_value=(None, "network_failure")
        ) as post:
    ...
    def test_migrates_existing_outbox_and_advances_before_crash(self):
        ...
        with (
            patch.object(upload, "post", side_effect=RuntimeError("crash")),
            self.assertRaises(RuntimeError),
        ):
            upload.sync(self.state, limit=1)
        with sqlite3.connect(self.state / "observations.sqlite3") as db:
            self.assertEqual(
                db.execute("SELECT * FROM upload_outbox ORDER BY seq").fetchall(),
                before,
            )
        self.assertEqual(self.sync_process()["delivered"], 1)
    ...
    def test_concurrent_uploader_cannot_advance_cursor(self):
        ...
        with observer.locked(self.state / "uploader"):
            ...
            with sqlite3.connect(self.state / "observations.sqlite3") as db:
                self.assertEqual(
                    db.execute("SELECT seq FROM upload_attempt_cursor").fetchone()[0], 0
                )
        self.assertEqual(self.sync_process()["delivered"], 1)
