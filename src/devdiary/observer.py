...
def document(raw):
    ...
    if not isinstance(result, dict) or not isinstance(result.get("hooks", {}), dict):
        raise TypeError("invalid_settings")
...
def groups_for(executable, state, installation_id, vendor="claude"):
    ...
    return {
        event: {"hooks": [{"type": "command", "command": command, "timeout": 1}]}
        for event in observer_hook.VENDORS[vendor_name(vendor)][1]
    }
...
def plan(settings, state, repository, executable, binding=None, *, vendor="claude"):
    ...
    if binding is not None and (
        set(binding) != {"session_id", "agent_id", "actor_ref"}
        or not all(observer_hook.token(binding[k]) for k in ("session_id", "actor_ref"))
        or (
            binding["agent_id"] is not None
            and not observer_hook.token(binding["agent_id"])
        )
    ):
    ...
    return {
        ...
        "trust_action": "review_exact_definitions_in_codex_/hooks"
        if vendor == "codex"
        else "review_in_claude",
...
def owned_counts(config, owned):
    return [
        config.get("hooks", {}).get(event, []).count(group)
    ...
    with locked(state):
        ...
        if (
            ...
            if old.get("vendor", "claude") != selected:
                raise ValueError("vendor_mismatch")
            if owned_counts(document(read(settings)), old["groups"]) == [1] * len(
                old["groups"]
            ):
                return {"registration": "installed", "delivery": "local_only"}
        if (
            digest(read(settings)) != proposal["expected_settings_sha256"]
            or digest(read(state / "manifest.json"))
            != proposal["expected_manifest_sha256"]
        ):
            raise ValueError("stale_plan_replan")
        executable = trusted_executable(proposal["executable"])
        if proposal["groups"] != groups_for(
            executable, state, proposal["installation_id"], selected
        ):
        ...
        record = dict(
            proposal,
            status="prepared",
            settings_existed=read(settings) is not None,
    ...
    with locked(state):
        ...
        if not old or old["status"] == "removed":
            return {
                ...
                "retention": "local_observations_retained",
            }
        ...
        raw = read(settings)
        config = document(raw)
        if old["status"] == "removing" and (
            digest(raw) == old.get("removed_settings_sha256")
            or (raw is None and not old["settings_existed"])
        ):
            old["status"] = "removed"
            save_manifest(state, old)
            return {
        ...
        if not config and not old["settings_existed"] and read(settings) == encode({}):
            settings.unlink()
        old["status"] = "removed"
        save_manifest(state, old)
    ...
    with closing(
        ...
        return [
            ...
            for row in db.execute(
                "SELECT metadata FROM observations ORDER BY seq DESC LIMIT ?",
                (min(max(int(limit), 1), observer_hook.MAX_ROWS),),
            )
        ][::-1]
...
def health(state):
    ...
    result = {
        ...
        "trust": "not_observable_verify_in_host",
        ...
        "limitations": [
            "no_remote_upload",
            "no_git_attribution_or_human_minutes",
            "no_duration_inference",
            "no_global_settings_or_policy_visibility",
    ...
    if vendor == "codex":
        ...
        result["limitations"].extend(
            [
                "codex_feature_and_managed_policy_not_inspected",
    ...
    if old["status"] == "installed":
        if owned_counts(config, old["groups"]) != [1] * len(old["groups"]):
            ...
            config.get("disableAllHooks") is True
            or config.get("allowManagedHooksOnly") is True
        ):
    ...
    try:
        ...
        database = state / "observations.sqlite3"
        with closing(
            sqlite3.connect(database.as_uri() + "?mode=ro", uri=True, timeout=0.05)
        ) as db:
            count = db.execute(
        ...
        result.update(
            ...
            coverage="observed" if rows else "awaiting_events",
        )
        sessions = {}
        for row in rows:
        ...
        for session in sessions.values():
            session["lifecycle"] = (
                (
                    "end_observed"
                    if session["start_observed"]
                    else "incomplete_missing_start"
                )
                if session["end_observed"]
                else (
                    "open_or_incomplete"
                    if session["start_observed"]
                    else "incomplete_missing_start_and_end"
                )
            )
