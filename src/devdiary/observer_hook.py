...
def connect(state):
    ...
    if database.exists() and (
        not database.is_file() or database.stat().st_size > MAX_DB
    ):
    ...
    db.execute(
        "CREATE TABLE IF NOT EXISTS observations (seq INTEGER PRIMARY KEY, dedup TEXT UNIQUE, metadata TEXT NOT NULL)"
    )
    db.execute(
...
def dropped(db):
    db.execute(
        "INSERT INTO counters VALUES ('dropped', 1) ON CONFLICT(name) DO UPDATE SET value=value+1"
    )
    db.commit()
...
def _collect(state, installation_id, stream):
    ...
    try:
        ...
        try:
            ...
            keys = (
                ("turn_id", "tool_use_id", "agent_id", "agent_type")
                if vendor == "codex"
                else ("prompt_id", "tool_use_id", "agent_id", "agent_type")
            )
            for key in keys:
                if key in data:
            ...
            if event == "SessionEnd" and data.get("reason") in (
                "clear",
                "resume",
                "logout",
                "prompt_input_exit",
            ...
            if event in ("PreToolUse", "PostToolUse", "PostToolUseFailure") and token(
                data.get("tool_name")
            ):
            ...
            if (
                ...
                and binding["agent_id"] == result.get("agent_id")
            ):
                result.update(
                    actor_ref=binding["actor_ref"],
                    attribution_basis="explicit_local_binding",
                )
            ...
            if event in ("PreToolUse", "PostToolUse", "PostToolUseFailure"):
                ...
                if vendor == "codex":
                    ...
                    identity = (
                        [identity, result["turn_id"]]
                        if identity and result.get("turn_id")
                        else None
                    )
            dedup = (
                hashlib.sha256(
            ...
            result["deduplication"] = (
                "vendor_correlation" if identity else "unavailable"
            )
            ...
            if (
                ...
                return
            db.execute(
                "INSERT OR IGNORE INTO observations(dedup, metadata) VALUES (?, ?)",
                (dedup, json.dumps(result, sort_keys=True)),
            )
            db.commit()
...
def main():
    ...
    except BaseException:  # noqa: BLE001 - host hooks must remain failure-neutral
        # Never log private hook data or emit a host decision/context, even on exit.
        return 0
    finally:
