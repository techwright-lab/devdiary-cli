...
FIELDS = ('schema_version', 'observation_id', 'installation_id', 'runtime',
          'session_id', 'event', 'observed_at', 'attribution_basis', 'actor_ref',
          'prompt_id', 'turn_id', 'tool_use_id', 'agent_id', 'agent_type', 'model',
          'tool_name', 'source', 'reason', 'repository')
...
def validate_endpoint(value):
    ...
    if (parsed.username or parsed.password or parsed.query or parsed.fragment or
            parsed.path != '/ingest/v1/observations' or not parsed.hostname or
...
def scope(config):
    return observer.digest(observer.encode({k: v for k, v in config.items() if k != 'key_file'}))


def database(state):
    db = observer_hook.connect(state)
    db.execute('CREATE TABLE IF NOT EXISTS upload_cursor (singleton INTEGER PRIMARY KEY CHECK(singleton=1), seq INTEGER NOT NULL)')
    ...
    db.execute('CREATE TABLE IF NOT EXISTS upload_attempt_cursor (singleton INTEGER PRIMARY KEY CHECK(singleton=1), seq INTEGER NOT NULL)')
    ...
    db.execute('CREATE INDEX IF NOT EXISTS upload_pending ON upload_outbox(delivered, seq)')
    db.commit()
    return db


def configure(state, *, endpoint, collector_ref, repository_ref, key_file, consent=False):
    if not consent:
        raise ValueError('consent_required')
    state = observer.absolute(state)
    validate_endpoint(endpoint)
    if (not re.fullmatch(r'https://github\.com/[A-Za-z0-9_-]+/[A-Za-z0-9_.-]+', repository_ref)
            or repository_ref.endswith(('.git', '/.', '/..')) or not observer_hook.token(collector_ref)):
    ...
    with observer.locked(state / 'uploader'), observer.locked(state):
        ...
        config = {'schema_version': 1, 'endpoint': endpoint, 'collector_ref': collector_ref,
                  'repository_ref': repository_ref, 'repository': m['repository'],
                  'installation_id': m['installation_id'], 'key_file': str(key_file)}
        with closing(database(state)) as db:
            ...
            if db.execute('SELECT 1 FROM upload_outbox WHERE scope != ? LIMIT 1', (scope(config),)).fetchone():
                raise ValueError('frozen_connection_conflict')
        path = state / CONNECTION
        observer_hook.safe_path(path)
        observer.atomic(path, observer.encode(config), observer.digest(observer.read(path)))
    return {'connection': 'configured', 'delivery': 'explicit_sync_only'}


def envelope(row, config):
    if row.pop('repository') != config['repository'] or row['installation_id'] != config['installation_id']:
        raise ValueError('installation_scope_conflict')
    row['repository_ref'] = config['repository_ref']
    micros = int((Decimal(str(row['observed_at'])) * 1000000).to_integral_value(rounding=ROUND_HALF_EVEN))
    row['observed_at'] = (datetime(1970, 1, 1, tzinfo=UTC) + timedelta(microseconds=micros)).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
    payload = json.dumps(row, sort_keys=True, separators=(',', ':'), allow_nan=False).encode()
...
def freeze(db, config, limit):
    ...
    projection = ','.join(f"json_extract(metadata, '$.{field}')" for field in FIELDS)
    db.execute('BEGIN IMMEDIATE')
    try:
        cursor = db.execute('SELECT seq FROM upload_cursor WHERE singleton=1').fetchone()[0]
        rows = db.execute(f'SELECT seq,{projection} FROM observations WHERE seq > ? ORDER BY seq LIMIT ?', (cursor, limit)).fetchall()
        for values in rows:
            row = {field: value for field, value in zip(FIELDS, values[1:]) if value is not None}
            payload = envelope(row, config)
            db.execute('INSERT INTO upload_outbox(seq,scope,endpoint,collector_ref,payload) VALUES (?,?,?,?,?)',
                       (values[0], scope(config), config['endpoint'], config['collector_ref'], payload))
...
def receipt(raw, payload, collector_ref):
    ...
    if (not isinstance(value, dict) or set(value) != {*expected, 'record_id'} or
            any(value[k] != v for k, v in expected.items()) or
            type(value['record_id']) is not int or value['record_id'] <= 0):
...
def post(endpoint, key, payload, collector_ref):
    ...
    try:
        request = urllib.request.Request(validate_endpoint(endpoint), data=payload, method='POST',
...
def acknowledge(db, seq, value):
    db.execute('UPDATE upload_outbox SET delivered=1, receipt=?, failure=NULL WHERE seq=?',
...
def pending_batch(db, limit):
    ...
    cursor = db.execute('SELECT seq FROM upload_attempt_cursor WHERE singleton=1').fetchone()[0]
    query = 'SELECT seq,endpoint,collector_ref,payload FROM upload_outbox WHERE delivered=0 AND '
    rows = db.execute(query + 'seq > ? ORDER BY seq LIMIT ?', (cursor, limit)).fetchall()
    if len(rows) < limit:
        rows += db.execute(query + 'seq <= ? ORDER BY seq LIMIT ?', (cursor, limit - len(rows))).fetchall()
    return rows
    ...
    with observer.locked(state / 'uploader'):
        ...
        with closing(database(state)) as db:
            ...
            for seq, endpoint, collector_ref, payload in rows:
                ...
                db.execute('UPDATE upload_attempt_cursor SET seq=? WHERE singleton=1', (seq,))
                db.commit()
                value, failure = post(endpoint, key, payload, collector_ref)
                if failure:
                    db.execute('UPDATE upload_outbox SET failure=? WHERE seq=?', (failure, seq))
                    db.commit()
                    # Shared outages should not hammer the endpoint. Row-specific
                    # failures still allow the rest of this bounded page through.
                    if failure in ('http_401', 'http_403', 'http_429', 'network_failure') or failure.startswith('http_5'):
                        break
                    continue
...
def health(state):
    ...
    with closing(sqlite3.connect(path.as_uri() + '?mode=ro', uri=True, timeout=0.05)) as db:
        ...
        if not db.execute("SELECT 1 FROM sqlite_master WHERE name='upload_outbox'").fetchone():
            return {'pending': total, 'delivered': 0, 'last_failure': None}
        delivered = db.execute('SELECT count(*) FROM upload_outbox WHERE delivered=1').fetchone()[0]
        failure = db.execute('SELECT failure FROM upload_outbox WHERE failure IS NOT NULL ORDER BY seq DESC LIMIT 1').fetchone()
        return {'pending': total - delivered, 'delivered': delivered,
