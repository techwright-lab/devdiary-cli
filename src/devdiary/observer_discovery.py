...
def classify(vendor, version_output, help_output):
    ...
    detected = (
        "--dangerously-bypass-hook-trust" in help_output
        if vendor == "codex"
        else all(flag in help_output for flag in ("--settings", "--setting-sources"))
    )
    return {
        "vendor": vendor,
        "runtime": "codex" if vendor == "codex" else "claude-code",
        "version": version,
        "support": (
            "unknown_version"
            if not version
            else "documented_interface_detected_runtime_unqualified"
            if detected
            else "unsupported_interface"
        ),
        "runtime_qualified": False,
        "trust": "not_inspected",
        "feature_policy": "not_inspected",
        "registration": "not_inspected",
...
def probe(executable, argument, home):
    from devdiary.observer import require_platform
...
def discover(vendor=None):
    ...
    for name in vendors:
        ...
        result.update(
            executable=executable, support="not_installed", evidence="not_probed"
        )
        if executable:
            try:
